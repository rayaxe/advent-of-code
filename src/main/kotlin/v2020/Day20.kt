        ...
        for (x in puzzle[0].indices) {
            if (x == 0) {
                ...
                if (x < puzzleWidth - 1) {
                    nextMatch =
                        tile.matches.first {
                            it.borderFrom.orientation == nextMatch.borderTo.orientation.rotate().rotate()
                        }
                }
            }
        }
...
private fun findTopLeftCornerMatch(borders: MutableList<Match>): Match {
    val orientationsOnSameSide = listOf(Orientation.UP, Orientation.RIGHT, Orientation.DOWN, Orientation.LEFT)
    val (match1, match2) = borders.filter { it.borderFrom.orientation in orientationsOnSameSide }
